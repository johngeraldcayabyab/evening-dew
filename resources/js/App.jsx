...
manifests.forEach((manifest) => {
    const manifestRoutes = manifest.routes;
    const hasTable = manifestRoutes.includes(HAS_TABLE);
    if (hasTable) {
        routes.push({
            path: `${manifest.moduleName}`,
            key: uuidv4(),
            element: <CustomTable {...manifest} key={`${manifest.moduleName}-${manifest.displayName}-table`}/>,
            loader: async ({request, params}) => {
                const url = new URL(request.url);
                console.log(paramsToObject(url.searchParams));
                // const searchTerm = url.searchParams;
                return fetch(`/api/${manifest.moduleName}`);
            }
        });
    }
})
...
const router = createBrowserRouter([{
    element: <ContentContainer routes={routes}/>,
    errorElement: <ErrorPage/>,
    children: routes
}]);
