...
import AllSaleForm from './AllSaleForm';
    ...
    return (
        <Switch>
            <Route exact key={uuidv4()} path={`/${displayName}`}
                   render={props => <AllSaleTable key={props.location.key}/>}
            />
            <Route exact key={uuidv4()} path={`/${displayName}/create`}
                   render={props => <AllSaleForm key={props.location.key}/>}
            />
